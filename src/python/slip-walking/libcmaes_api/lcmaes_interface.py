...
import lcmaes
from . import cma_multiplt as cmaplt
...
def get_best_candidate(res):
    """
    Extracts the best candidate from the optimization result.

    Parameters:
    - res: The result object from the optimization process.

    Returns:
    - A tuple containing the best candidate's position (as a list) and its objective function value.
    """
    bcand = res.best_candidate()
    bx = lcmaes.get_candidate_x(bcand)  # Best candidate's position
    bf = bcand.get_fvalue()  # Best candidate's objective function value
    return bx, bf
